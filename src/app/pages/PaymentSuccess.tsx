...
async function fetchPaymentMessage(path: string): Promise<string> {
  ...
  return typeof data === "string" && data.trim() ? data.trim() : "Payment successful.";
}
...
export default function PaymentSuccess() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const result = await fetchPaymentMessage("/api/v1/payment/test-success");
        if (!active) return;
        setMessage(result);
      } catch {
        if (!active) return;
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 backdrop-blur-sm p-6">
        <Card className="w-full max-w-md rounded-2xl border border-green-200 bg-white p-8 shadow-2xl">
